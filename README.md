...
* Try to foster a culture of improvement, benchmark your organisation against some form of [maturity model](https://devopsadoptmeth.wordpress.com/method-description/devops-maturity-model/) to see the gaps and try to close them.
...
* If there are problems that you have to work around in your code then link to some permanent record (e.g. a URL of a Jira story or [ADR](https://github.com/joelparkerhenderson/architecture_decision_record)) of why, the following code comment cause me a lot of running around (The `git blame' gave me a commit that lead to a PR that had zero details in it, authored by someone who was not available.):
```
